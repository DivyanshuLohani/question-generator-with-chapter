...
class Html2Latex(object):
    ...
    def element2latex(self, el, cascading_style, selectors):
        ...
        if not ignoreContent:
            if el.text:
                text = self.inside_characters(
                    el, el.text, leaveText, ignoreContent)
                r = self.replacements_head.get(el, None)
                if r:
                    text = re.sub(r[0], r[1], text)
                result.append(text)
            for child in el:
                result.append(self.element2latex(
                    child, cascading_style, selectors))
                if child.tail:
    ...
    def run(self):
        ...
        htmlParser = lxml.html.HTMLParser(
        ...
        for i in range(0, len(self.inputFiles)):
            ...
            if not self.disable_style_tags:
                documentStyle = '\n'.join(
                    [style.text for style in root.getchildren()[0].findall('style')])
            stylesheet = styleparser.parseString(
                externalStyle + documentStyle, href="file://"+os.path.abspath(inputFile))
            cascading_style = get_view(
                root, stylesheet, style_callback=styleattribute)
